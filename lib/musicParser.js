...
// ---------------------------------------------------------------------------
// sound/music_player_table.inc  —  per-player track budgets (NUM_TRACKS_*)
// ---------------------------------------------------------------------------

/**
 * Parse `.equiv NUM_TRACKS_BGM, 10` style lines into { MUSIC_PLAYER_BGM: 10, ... }.
 * MPlayStart never starts tracks beyond the player's budget, so tracks past it are
 * silent in-game — the editor needs the numbers to warn (and mute) accurately.
 */
function parseTrackBudgets(content) {
    const budgets = {};
    const re = /^\s*\.equiv\s+NUM_TRACKS_([A-Za-z0-9_]+)\s*,\s*(\d+)/gm;
    let match;
    while ((match = re.exec(content)) !== null) budgets['MUSIC_PLAYER_' + match[1]] = parseInt(match[2], 10);
    return budgets;
}
