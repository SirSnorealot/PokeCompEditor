...
class PokemonEditorPanel {
    ...
    _assetsFor(item, index) {
        const frontSymbols = String(item.fields.frontPic.value || '').match(/gMonFrontPic_[A-Za-z0-9_]+/g) || [];
        const directories = new Set();
        for (const symbol of frontSymbols) for (const relative of index[symbol] || []) directories.add(path.posix.dirname(relative));
        if (!directories.size) {
        ...
        for (const relativeDir of directories) {
            ...
            for (const name of fs.readdirSync(absoluteDir)) {
                ...
                const relativePath = relativeDir + '/' + name, absolutePath = path.join(absoluteDir, name);
        ...
        return assets.sort((a, b) => assetOrder(a.name) - assetOrder(b.name) || a.name.localeCompare(b.name));
    }
...
(function () {
    ...
    function processIcon(rawSource, frameHeight, callback, forceFirstColorTransparent) {
        var cacheKey = rawSource + '|' + frameHeight + '|' + (forceFirstColorTransparent ? 'keyed' : 'normal');
    ...
    function makePokedexSizePreview(item) {
        ...
        var asset = frontAsset(item); if (asset) processIcon(asset.uri, 64, function (source) { mon.src = source; }, true);
    ...
    function makeBattlePreview(item) {
        ...
        var asset = frontAsset(item), back = backAsset(item); if (asset) processIcon(asset.uri, 64, function (source) { frontSprite.src = source; }, true); if (back) processIcon(back.uri, 64, function (source) { backSprite.src = source; }, true);
    ...
    function renderGraphics(item, root) {
        ...
        item.assets.forEach(function (asset) {
            ...
            if (asset.image) {
                var image = document.createElement('img'); image.className = 'preview'; image.src = asset.uri; image.alt = asset.name; preview.appendChild(image);
