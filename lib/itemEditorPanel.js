...
class ItemEditorPanel {
    ...
    _load(selectedItemId) {
        try {
            ...
            setTimeout(() => this._panel.webview.postMessage({
                type: 'init', items, fields: ITEM_FIELDS, fieldOptions: this._fieldOptions(items),
                icons: this._iconMap(items), selectedItemId,
            }), 100);
        } catch (error) {
    ...
    _fieldOptions(items) {
        const root = this._projectRoot.fsPath;
        const uniqueValues = field => [...new Set(items.map(item => item.fields[field].value)
            .filter(value => value && !value.includes('\n') && !value.includes('#')))].sort();
        const constants = (filePath, pattern, excluded = []) => {
            if (!fs.existsSync(filePath)) return [];
            const values = [...fs.readFileSync(filePath, 'utf8').matchAll(pattern)].map(match => match[0]);
            return [...new Set(values)].filter(value => !excluded.includes(value)).sort();
        };
        const itemConstants = path.join(root, 'include', 'constants', 'items.h');
        const graphicsFile = path.join(root, 'src', 'data', 'graphics', 'items.h');
        return {
            pocket: constants(path.join(root, 'include', 'constants', 'item.h'), /\bPOCKET_[A-Z0-9_]+\b/g,
                ['POCKETS_COUNT', 'POCKET_DUMMY']),
            sortType: constants(path.join(root, 'include', 'item.h'), /\bITEM_TYPE_[A-Z0-9_]+\b/g),
            type: mergeOptions(constants(itemConstants, /\bITEM_USE_[A-Z0-9_]+\b/g), uniqueValues('type')),
            fieldUseFunc: uniqueValues('fieldUseFunc'),
            battleUsage: constants(itemConstants, /\bEFFECT_ITEM_[A-Z0-9_]+\b/g,
                ['EFFECT_ITEM_COUNT']),
            secondaryId: uniqueValues('secondaryId'),
            effect: uniqueValues('effect'),
            holdEffect: constants(path.join(root, 'include', 'constants', 'hold_effects.h'),
                /\bHOLD_EFFECT_[A-Z0-9_]+\b/g, ['HOLD_EFFECT_COUNT']),
            importance: mergeOptions(['0', '1', '2', '3'], uniqueValues('importance')),
            notConsumed: ['FALSE', 'TRUE'],
            iconPic: constants(graphicsFile, /\bgItemIcon_[A-Za-z0-9_]+(?=\[\])/g),
            iconPalette: constants(graphicsFile, /\bgItemIconPalette_[A-Za-z0-9_]+(?=\[\])/g),
        };
    }
...
<script nonce="${nonce}">
(function(){
'use strict';var vscode=acquireVsCodeApi(),items=[],fieldNames=[],fieldOptions={},icons={},selected=-1,edits={};
...
function makeFieldControl(name,value){var options=fieldOptions[name];if(!options)return name==='description'?document.createElement('textarea'):document.createElement('input');var select=document.createElement('select');if(!value){var blank=document.createElement('option');blank.value='';blank.textContent='\u2014 not set \u2014';select.appendChild(blank)}if(value&&options.indexOf(value)<0)options=[value].concat(options);options.forEach(function(optionValue){var option=document.createElement('option');option.value=optionValue;option.textContent=optionValue;select.appendChild(option)});return select}
function select(index){commit();selected=index;var item=items[index];el('empty').style.display='none';el('editor').style.display='flex';el('title').textContent=item.displayName;el('item-id').textContent=item.id;var src=icon(item);el('large-icon').src=src;el('large-icon').style.visibility=src?'visible':'hidden';var container=el('fields');container.textContent='';fieldNames.forEach(function(name){var data=item.fields[name],wrap=document.createElement('div');wrap.className='field'+(name==='description'?' wide':'');var label=document.createElement('label');label.textContent=labels[name]||name.charAt(0).toUpperCase()+name.slice(1);var reason=document.createElement('small');if(!data.present)reason.textContent='not set';else if(data.conditional)reason.textContent='conditional';label.appendChild(reason);var value=(edits[item.id]&&edits[item.id][name]!==undefined)?edits[item.id][name]:data.value;var input=makeFieldControl(name,value);input.dataset.field=name;input.value=value;input.disabled=data.conditional;input.addEventListener(input.tagName==='SELECT'?'change':'input',commit);wrap.appendChild(label);wrap.appendChild(input);container.appendChild(wrap)});el('raw').textContent=item.rawBody;renderList();var active=document.querySelector('.item.active');if(active)active.scrollIntoView({block:'nearest'});status()}
...
function mergeOptions(first, second) {
    return [...new Set(first.concat(second))].sort();
}
